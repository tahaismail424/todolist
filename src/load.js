...
function makeNewRadio(degree, prio) {
    ...
    (prio) ? inputP.setAttribute('name', 'priority') :
...
function toDoPrompt (task) {
    ...
    for (let i = 0, n = projList.length; i < n; i++) {
        ...
        newForm.insertBefore(document.createElement('br'), newForm.children[12 + i]);
    }

    //for EDIT task 
    if (task) newForm.addEventListener('submit',
    (e) => {
        saveToDo(task);
        enableButtons();
        e.target.remove();
    ...
    //for NEW task
    else newForm.addEventListener('submit',
    (e) => {
        saveToDo();
        enableButtons();
        e.target.remove();
    ...
    for (let node of newForm.childNodes) {
        if (node.className === 'input-box') {
            for (let field of node.childNodes) {
                ...
                if (field.id === 'description') field.setAttribute('value', item.description);
...
function projPrompt(existingProj) {
    ...
    if (existingProj) newForm.addEventListener('submit',
    (e) => {
...
export {displayToDo, toDoEditPrompt, toDoPrompt, makeNew, newProject, displayProj, refreshDisplay, hideTasks}